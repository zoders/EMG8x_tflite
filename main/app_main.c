...
uint32_t pred_head(){
    ...
    D = D << 8;
    uint32_t endstr = (uint32_t)'\0';
    uint32_t pred = P | R | D | endstr;
    return (uint32_t)pred;
}
            ...
            {          
                if (drdy_thread_context.tail!=drdy_thread_context.head)
                {
                    ...
                    if (num_samples == 360){
                        ...
                        ESP_LOGI(TAG, "s %f", signal_[193-1]);
                        ESP_LOGI(TAG, "s %f", signal_[15693-1]);
                        ESP_LOGI(TAG, "s %f", signal_[2391-1]);
                        ESP_LOGI(TAG, "s %f", signal_[23038-1]);
                        ESP_LOGI(TAG, "s %f", signal_[23039-1]);
                        ESP_LOGI(TAG, "s %f", signal_[23040-1]);
                        ESP_LOGI(TAG, "Centerizing filter...");
                        // for (int i = 0; i < 23040; i++)
                        //     if (!b[i])
                        //         ESP_LOGI(TAG, "bool %d false", i);
                        except_empty_space(signal_, b, 23040);
                        centrize(signal_/*, sum_of_numbers*/, 23040);
                        ESP_LOGI(TAG, "s %f", signal_[193-1]);
                        ESP_LOGI(TAG, "s %f", signal_[15693-1]);
                        ESP_LOGI(TAG, "s %f", signal_[2391-1]);
                        ESP_LOGI(TAG, "s %f", signal_[23038-1]);
                        ...
                        ESP_LOGI(TAG, "[APP] Free memory: %d bytes", esp_get_free_heap_size()) ;
                        signal_filtered = (float*) malloc(  23040 * sizeof(float) );
                        ESP_LOGI(TAG, "Starting fiter...");
                        ESP_LOGI(TAG, "[APP] Free memory: %d bytes", esp_get_free_heap_size()) ;
                        convolve(signal_, signal_filtered, 23040);
                        free(signal_);
                        
                        float *muaps;
                        muaps = muap(signal_filtered, 23040);
                        //ESP_LOGI(TAG, "[APP] Free memory: %d bytes", esp_get_free_heap_size()) ;
                        ESP_LOGI(TAG, "Setuping TF Lite...");
                        setup();
                        portYIELD();
                        
                        ESP_LOGI(TAG, "Starting predication");
                        // 
                        uint32_t num_of_muaps = (uint32_t)muaps[0];
                        //ESP_LOGI(TAG, "[APP] Num of muaps: %d", num_of_muaps) ;
                        char nom_str[3];
                        snprintf(nom_str, 3, "%d", num_of_muaps);
                        if (num_of_muaps < 10){
                            nom_str[1] = nom_str[0];
                            nom_str[0] = '0';
                            nom_str[2] = '\0';
                        ...
                        int borders[2];
                        ESP_LOGI(TAG, "[APP] Num of muaps: %s", nom_str) ;
                        write (cs, nom_str, 3*sizeof(char));
                        for(int muap_num = 1; muap_num < muaps[0]*2+1; muap_num+=2){
                            
                            float output[3];
                            //output[0] = (float)pred_head();
                            predicate(signal_filtered, muaps[muap_num], muaps[muap_num + 1], output);
                            portYIELD();
                            // output[3] = (float)muaps[muap_num];
                            // output[4] =  (float)muaps[muap_num + 1];
                            
                            borders[0] = (int)muaps[muap_num];
                            borders[1] = (int)muaps[muap_num + 1];
                            ESP_LOGI(TAG, "[APP] muap borders: %d - %d", borders[0], borders[1]) ;
                            write(cs, "predication", 12*sizeof(char));
                            write(cs, output, 3*sizeof(float));
                            write(cs, borders, 2*sizeof(int));
                            vTaskDelay(100 / portTICK_PERIOD_MS) ;
                            //ESP_LOGI(TAG, "out [%lf, %lf, %lf]", output[0], output[1], output[2]);
                            //free(output);
                        }
                        //write(cs, "stop this operation", 20*sizeof(char));
                        free_tensor_arena();
                        // b = (bool*) malloc(  23040 * sizeof(bool) );
                        // for (int i = 0; i < 23040; i++)
                        //     b[i] = false;
                    }
                    samples64 = 0;
                    ...
                    /*if (num_samples == 361)
                        if((write(cs, "ALOHA bitches", sizeof(char)*13)) < 0) ESP_LOGE(TAG, "TCP_SERVER: Send failed ALOHA\n") ;*/
                }
                //taskYIELD() ;
...
//    int dmaChan             = 0 ;  // disable dma
 ...
 /*   while(1)
    ...
    // Configure channels
    ad1299_wreg( spi_dev, AD1299_ADDR_CH1SET, 0x30 ) ;      // CH1: Test signal,    PGA_Gain=1
...
IRAM_ATTR void spi_data_pump_task( void* pvParameter )
{
    ...
    while(1)
    {
        ...
        for(int chNum=0;chNum<1;chNum++)
        {
			...
			// loop();
            //if (chNum == 0)
            //ESP_LOGI("var: ", "%f", (float)value_i32);
#if CONFIG_EMG8X_BOARD_EMULATION
            value_i32                   = (drdy_thread_context.sampleCount%20)<10 ;
#endif
            if ((0 <= num_samples && num_samples <= 359)){
                // ESP_LOGI(TAG, "%f: %d, %d", 
                // (float)value_i32, drdy_thread_context.head, drdy_thread_context.sampleCount);
                //num_of_points++;
                //sum_of_numbers += (double)value_i32;
                signal_[num_samples*64 + drdy_thread_context.sampleCount]
                    = (float)value_i32;
                b[num_samples*64 + drdy_thread_context.sampleCount] = true;
                // if (num_samples > 3)
                // ESP_LOGI(TAG, " sobir: sig[%d] =  %f",num_samples*64 + drdy_thread_context.sampleCount,
                //  (float)value_i32) ;
                //  vTaskDelay(10 / portTICK_PERIOD_MS) ;
                // ESP_LOGI(TAG, "%f: %d, %d", (float)value_i32,
                // drdy_thread_context.head, drdy_thread_context.sampleCount);;
            }
            drdy_thread_context.adcDataQue[drdy_thread_context.head][
                (CONFIG_EMG8X_TRANSPORT_BLOCK_HEADER_SIZE)/sizeof(int32_t) +
            ...
            // signal_[num_samples+samples64] = value_i32 ;
            // samples64++;
            // if (samples64 == 64)
            //     samples64 = 0;
            // num_of_points++;
            // if (num_samples == 2)
            //     ESP_LOGI("sigs", "%d %f",num_samples, (float)value_i32);
            // if (num_samples == 240)
            //     ESP_LOGI("sigs", "%d %f",num_samples, (float)value_i32);
            // if (num_samples == 361)
            //     ESP_LOGI("sigs", "%d %f",num_samples, (float)value_i32);
            // if (num_samples == 362)
            //     ESP_LOGI("sigs", "%d %f",num_samples, (float)value_i32);
            // if ((0 <= num_samples) && (num_samples <= 363) && samples64 < 64){
            //     num_of_points++;
            //     samples64++;
            // }
        }
...
void app_main()
{
    ...
    for (int i = 0; i < 23040; i++)
        b[i] = false;
    ESP_LOGI(TAG, "[APP] Free memory: %d bytes", esp_get_free_heap_size()) ;
    signal_ = (float*) malloc(  23040 * sizeof(float) );
    ESP_LOGI(TAG, "[APP] Allocating data for signal") ;
    ESP_LOGI(TAG, "[APP] Free memory: %d bytes", esp_get_free_heap_size()) ;
    ESP_LOGI(TAG, "[APP] Free memory: %d bytes", esp_get_free_heap_size()) ;
    ESP_LOGI(TAG, "[APP] Free memory: %d bytes", esp_get_free_heap_size()) ;
